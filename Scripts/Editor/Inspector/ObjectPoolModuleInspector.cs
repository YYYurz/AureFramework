            ...
            if (currentState)
            {
                EditorGUILayout.BeginVertical("box");
                {
                    ...
                    if (objectInfos.Length > 0) {
                        ...
                        {
                            EditorGUILayout.LabelField(string.IsNullOrEmpty(objectInfo.Name) ? "<None>" : objectInfo.Name, $"{objectInfo.IsInUse}\t{objectInfo.IsLock}\t{objectInfo.LastUseTime.ToLocalTime()}");
                        }
                    }
                    else
                    {
